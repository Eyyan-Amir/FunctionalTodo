...
function App() {
  ...
  //   let checkOutTodo = todos[index];
  //   checkOutTodo = {
  //     ...checkOutTodo,
  //     onActive: true,
  //   }
  //   todos.splice(index, 1, checkOutTodo);
  //   setTodos([...todos]);
  //       };
        
  // const handleCheckBox = ()=>{
    //   var x = document.getElementById("createIcon");
    //   var y = document.getElementById("closeIcon");
    //   var z = document.getElementById("doneIcon");
    //   if (x.style.display === "block" && y.style.display && z.style.display) {
    ...
    return <List >
      {todos.map((t, i) => {
        ...
        return <ListItem key={i}>
          {/* <input type = "checkbox"  onClick = {() =>handleCheckBox(t)}/> */}
          <Checkbox
        checked={selectTodoIndex == i}
        onChange={() =>handleCheckBox(t)}
      />
          <ListItemText className={`${classes.line} ${t.status == 1 ? classes.throught : ""} `} primary={t.title} />
          <ListItemText className={`${classes.line} ${isChecked ? classes.unHide : classes.hide} `} primary={
          <React.Fragment >
          ...
          />
          {/* <ListItemText className={`${classes.line} ${t.onActive == true? classes.unHide : classes.hide} `} primary={
          <React.Fragment >
          <CreateIcon  className={classes.icon} onClick={() => editTodo(t, i)}/>
          <CloseIcon className={classes.icon} onClick={() => removeTodo(t)} />
          <DoneIcon className={classes.icon} onClick={() => completeTodo(t)}/>
          ...
          <ListItemIcon onClick={() => completeTodo(t)}>
    ...
    <div className={classes.root}>
      ...
      <Grid container className={classes.containerClass} >
        <Grid item xs={5} sm={5} md={5} lg={5}>
          ...
          <Card className={classes.cardBackground} >
          
            {
              screen == 0 && <>
                <CardContent className={classes.CardContnt}>
                  <Typography className={classes.todoHeader} color="textSecondary"  >{` ${count === 0 ? `you have ${count} todo :- ( ` : `you have ${count} todo :- ) `
                    }`} </Typography>
                  {/* <a className = {classes.clearBtn} href="#" onClick={clear}>Clear</a> */}
                  {/* <a className={classes.completeHistory} href="#" onClick={completeHistory}> Complete History</a> */}
            ...
            {screen == 1 && <> <CardContent>
              <ArrowBackIcon onClick={onShowHist} />
              <Typography className={classes.todoHeader} color="textSecondary" >
                {` ${delCount === 0 ? ` ${delCount} Delete todo  ` : `you have ${delCount} Delete todos `}`} </Typography>
              <div>
                {deleteList()}
              </div>
            </CardContent></>
            }
            {screen == 2 && <> <CardContent>
              <ArrowBackIcon onClick={onShowHist} />
              <Typography className={classes.todoHeader} color="textSecondary"  >
                {` ${compCount === 0 ? ` ${compCount} Complete todo  ` : `you have ${compCount} Complete todos `}`} </Typography>
              <div>
                {completeList()}
              </div>
            </CardContent></>
      ...
      <Menu
        ...
        <MenuItem onClick={deleteHistory}>  
           Delete History</MenuItem>
        <MenuItem onClick={completeHistory}>
          Complete History
          </MenuItem>
      </Menu>
          </Card>
